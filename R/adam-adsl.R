...
#          happens - explicitly, and flagged. Analysis dates are proper Date
#          columns (XPT v5 writes them as numeric DATE values), unlike the
#          all-character SDTM layer.
# ============================================================================

#' Derive ADSL, the subject-level analysis dataset
#'
#' Population flags: the extract collects no randomization record, so ITTFL
#' is "randomized" (has a planned arm) and SAFFL is "dosed" (has a treatment
#' start). Analysis dates are imputed per [impute_dtc()] and flagged; AGE
#' follows the imputed birth date, so a birth year alone is enough.
#'
#' @param dm The mapped SDTM DM dataset
...
derive_adsl <- function(dm, ex, ds) {
  ...
  # Disposition: the DISPOSITION EVENT record carries end-of-study; the
  # screen failures' PROTOCOL MILESTONE record is their last contact instead
  disposition <- ds |>
  ...
  dm |>
    ...
    transmute(
      ...
      # Birth date: SDTM kept it partial; ADSL resolves it per the
      # imputation rule and flags what was filled
      BRTHDTC,
      BRTHDT,
      BRTHDTF,
      ...
      # Population flags (see above). Safety requires randomization in this
      # design, which the ADaM validator asserts.
      ...
      # Unlike DM's "Screen Failure" placeholder, ADaM carries the absence
      # of an arm as an absent value.
      TRT01P   = if_else(ITTFL == "Y", ARM, NA_character_),
      TRT01PCD = if_else(ITTFL == "Y", ARMCD, NA_character_),
      TRT01A   = TRT01P,
      TRT01ACD = TRT01PCD,

      # Analysis dates with their imputation flags
      ENRLSTDT,
      TRTSDT, TRTSDTF,
      TRTEDT, TRTEDTF,
      ...
      # imputed and flagged like every other analysis date
      DTHDT  = DTHDT,
      DTHDTF = DTHDTF,
