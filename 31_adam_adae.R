...
# Input:   data/sdtm/{ae,suppae}.rds, data/adam/adsl.rds
# Output:  data/adam/adae.rds, data/adam/xpt/adae.xpt
# ============================================================================
...
# (IDVARVAL). The validator keeps the long format unique on this key, so the
# pivot cannot silently drop or duplicate qualifiers.
supp_labels <- suppae |>
...
# Every AE record must come with its qualifiers - a missing row here means
# the SUPPAE join or the pivot drifted.
unqualified <- ae |>
...
adae <- ae |>
  ...
  transmute(
    STUDYID, USUBJID,
    # One analysis record per collected event, so the SDTM sequence is the
    # analysis sequence - no renumbering to drift apart.
    ASEQ = AESEQ,
    AETERM, AEDECOD, AEBODSYS, AESEV, AESER, AEREL, AEACN, AEOUT,

    # The non-standard CRF fields, back where analysis wants them. AESI is
    # the investigator's judgement - its QEVAL = "INVESTIGATOR" lives on the
    # SUPPAE record this pivot came from.
    AESI, AEDISCON,

    # The ADSL dates the TRTEMFL window is defined against, carried on the
    # dataset (OCCDS convention) so the flag is checkable in place. SAFFL
    # rides along: an undosed subject's events can never be TE.
    SAFFL, TRTSDT, TRTEDT,

    # Analysis dates, imputed per the ADSL rule and flagged. Ongoing events
    # keep a missing AENDT - no fabricated end date, same as SDTM.
    ASTDT, ASTDTF,
    AENDT, AENDTF,

    # Analysis study days, anchored on TRTSDT and computed from the imputed
    # dates - unlike SDTM AESTDY, which stays NA for a partial start.
    ASTDY = derive_dy_d(ASTDT, TRTSDT),
    AENDY = derive_dy_d(AENDT, TRTSDT),

    # Treatment-emergent: onset (imputed start) on or after first dose and
    # up to the last dose. No grace window - no SAP defines one, so the
    # window is stated here where it can be argued with. The imputed date
    # is what classifies, and ASTDTF says when that happened. Events with
    # no classifiable onset (none collected, but the rule must say so) or
    # for undosed subjects stay blank.
    TRTEMFL = case_when(
...
write_sdtm(adae, "ADAE", rds_dir = paths$adam, xpt_dir = paths$xpt_adam)
