# ============================================================================
# Title:   Synthetic Medidata Rave clinical-view generator
# Purpose: Emit a fake SAS-on-Demand / Biostat Adapter "Regular" clinical view
#          extract - one CSV per CRF form plus the data dictionary and codelist
#          reference - so the SDTM scripts have source data without a live Rave.
# Output:  data/rave/{DM,VS,AE,CM,EX,DS}.csv
#          data/rave/ClinicalViewMetadata.csv
#          data/rave/CodeLists.csv
#
...
#   * field columns named from the field OID, with Rave suffix families
...
#
# Standalone use (00_setup.R not sourced):
#   Rscript -e 'source("05_generate_rave_extract.R")'
# Different seed / N:
#   Rscript -e 'source("00_setup.R"); source("05_generate_rave_extract.R"); \
#               generate_rave_extract(seed = 42, n = 60)'
# ============================================================================

if (!exists("paths")) {
  message("05_generate_rave_extract.R: 00_setup.R not sourced - standalone defaults")
  suppressPackageStartupMessages(library(tidyverse))
  paths       <- list(rave = file.path(getwd(), "data", "rave"))
  PROJECT     <- "SYNTH01"
  PROJECT_ID  <- "1487"
  STUDYID     <- "3021"
  ENVIRONMENT <- "Prod"
...
#' Rave date column family for one date field
#'
...
form_add <- function(e, sub, folder_oid, rec_date, fields,
  ...
  header <- list(
    userid = USER_ID, projectid = PROJECT_ID, project = PROJECT,
    studyid = STUDYID, environmentName = ENVIRONMENT,
...
form_write <- function(e, outdir) {
  ...
  cat("EOF\n", file = path, append = TRUE)   # Rave extracts end with an EOF row
  invisible(path)
}
...
#   oid    - Rave field OID (the CDISC LBTESTCD is assigned in 17_sdtm_lb.R)
#   std    - does Rave populate a converted _STD / _STD_UN column?
  ...
  for (idx in seq_along(subjects) - 1L) {
    ...
    for (folder in LB_VISIT_FOLDERS) {
      ...
      for (a in LB_PANEL) {
        ...
        if (a$std) {                          # Rave's configured conversion
  ...
  for (nm in names(forms)) {
    ...
    for (i in seq_along(all_cols)) {
      ...
      rows[[length(rows) + 1L]] <- tibble(
        ProjectName = PROJECT, FormOID = nm, FormName = e$page_name,
...
# that event: stamp the AE's log line on the CM record (CMSPID), which
# 23_sdtm_relrec.R turns into a RELREC pair. First match wins, in row
# order, and each AE is linked at most once - all deterministic.
...
# from MH_SEED + index; .Random.seed is restored afterwards, so the main
...
generate_rave_extract <- function(out = paths$rave, seed = RAVE_SEED, n = RAVE_N) {
  set.seed(seed)
...
# Regenerate on source(), mirroring how the SDTM scripts run on source().
generate_rave_extract()
